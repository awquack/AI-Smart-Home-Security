...
import database
import alerts
from face_recognition_module import check_frame_for_faces, FACE_RECOG_AVAILABLE
...
def run_detection(shared_state=None, show_window=True):
    ...
    print(f"  Faces  : {'ON (face_recognition)' if FACE_RECOG_AVAILABLE else 'OFF'}")
    ...
    while True:
        ...
        if confirmed and frames_since_yolo >= YOLO_EVERY:
            ...
            for det in new_yolo:
                ...
                if lbl.startswith("person") and FACE_RECOG_AVAILABLE:
                  try:
                    face_results = check_frame_for_faces(frame, (x, y, w, h))
                    for face in face_results:
                        ...
                        face_detections.append({
                            "box":   fbox,
                            "name":  fname,
                            "is_intruder": face["is_intruder"],
                            "time":  current_time,
                        })
                        if face["is_intruder"]:
                            fsnap = save_snapshot(frame, "intruder") if config.AUTO_SNAPSHOT else None
                            database.log_event(
                                "intruder",
                                label=f"Unknown face (conf={1-fdist:.0%})",
                                confidence=round(1.0 - fdist, 3),
                                snapshot_path=fsnap,
                                x=fbox[0], y=fbox[1], w=fbox[2], h=fbox[3],
                            )
                            alerts.send_alert(
                                "intruder",
                            ...
                            print(f"[FACE] INTRUDER — unknown face  dist={fdist:.2f}")
                        else:
                            database.log_event(
                                "face_recognized",
                                label=f"Recognized: {fname} (conf={1-fdist:.0%})",
                                confidence=round(1.0 - fdist, 3),
                                x=fbox[0], y=fbox[1], w=fbox[2], h=fbox[3],
                            )
                            print(f"[FACE] Recognized: {fname}  dist={fdist:.2f}")
                  except Exception as _fe:
                    print(f"[FACE] Error: {_fe}")
        ...
        # Motion boxes hidden — only YOLO and face boxes shown
