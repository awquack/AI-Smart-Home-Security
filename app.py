...
from face_recognition_module import encode_face_from_path, FACE_RECOG_AVAILABLE
...
@login_required
def snapshot_feed():
    """Single JPEG frame — polled by the dashboard canvas every 40ms."""
    jpeg = shared.get_jpeg(quality=90)
    if jpeg is None:
        # Return a simple black placeholder
        import numpy as np
        img = np.zeros((360, 640, 3), dtype="uint8")
        cv2.putText(img, "Starting camera...", (160, 180),
...
@login_required
def faces():
    enrolled = database.get_known_faces()
    return render_template("faces.html", faces=enrolled,
                           face_recog_available=FACE_RECOG_AVAILABLE,
                           config=config)


@app.route("/known_face_photo/<path:filename>")
@login_required
def known_face_photo(filename):
    base_dir  = os.path.dirname(os.path.abspath(__file__))
    faces_dir = os.path.join(base_dir, config.KNOWN_FACES_DIR)
...
def enroll_face():
    ...
    save_path = os.path.join(config.KNOWN_FACES_DIR, filename)
    photo.save(save_path)

    # Encode the face
    encodings = encode_face_from_path(save_path)
    if not encodings:
        os.remove(save_path)
        flash("No face detected in the photo. Please use a clear, front-facing image.", "warning")
        return redirect(url_for("faces"))
    if len(encodings) > 1:
        os.remove(save_path)
        flash("Multiple faces detected. Please upload a photo with only one person.", "warning")
        return redirect(url_for("faces"))

    database.add_known_face(name, encodings[0].tolist(), photo_path=save_path)
...
def delete_face(face_id):
    photo_path = database.delete_known_face(face_id)
    if photo_path and os.path.isfile(photo_path):
        os.remove(photo_path)
    flash("Face removed.", "success")
    return redirect(url_for("faces"))
